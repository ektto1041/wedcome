...
- 활성 영상은 Hero 전체에 `position: absolute; inset: 0`으로 배치합니다.
...
- 영상 교체 시 검은 프레임이 보이지 않도록 기존 Hero 이미지를 아래에 유지하고, 새 영상이 `canplay` 상태가 된 뒤 짧게 opacity를 전환합니다.
- 기존 Hero 그라데이션과 제목은 영상보다 위, 입력 레이어와 진행 바보다 아래 또는 의도한 z-index 순서에 배치합니다.
- 첫 영상은 즉시 로드하고 다음 영상은 현재 영상 재생이 안정된 뒤 미리 로드합니다. 모든 영상을 최초 진입에 동시에 내려받지 않습니다.
- 다음 영상 preload에 실패해도 현재 재생은 유지하며, 실제 전환 시 기본 로딩 상태로 대응합니다.
...
- reduced motion 환경에서는 영상 간 opacity 전환 등 부수적인 애니메이션도 제거합니다.
...
- 컴포넌트가 unmount될 때 animation frame, observer, preload용 media 객체와 이벤트 리스너를 정리합니다.
...
7. 현재 영상이 준비된 뒤 다음 영상을 preload하도록 연결했습니다.
